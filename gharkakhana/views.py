...
from gharkakhana.models import Customer, Restaurant, Order, FoodItem, Cart
...
def home(request):
        if 'id' in request.session.keys():
            if request.session['type'] == 'customer':
                ...
                return render(request, 'gharkakhana/userhome.html', context)
                ...
                return render(request, 'gharkakhana/resthome.html', context)
        else:
            return render(request, "gharkakhana/login.html")
...
def login(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if customer.check_password(password.encode('utf-8')):
            ...
            try:
                ...
                if restaurant.check_password(password.encode('utf-8')):
                    request.session['id'] = email
...
def signup(request):
    if request.method == 'POST':
        ...
        if usertype == 'Customer':
            ...
            user.set_password(user.make_password(password.encode('utf-8')))
            user.save()
            request.session['id'] = email
            request.session['type'] = 'customer'
...
def editDetails(request):
    if request.method == 'POST':
        if request.session['type'] == 'customer':
            ...
            return render(request, 'gharkakhana/userdetails.html', context)
        elif request.session['type'] == 'restaurant':
            restaurant = Restaurant.objects.get(email=request.session['id'])
            context = {
                'restaurant': restaurant,
            ...
            restaurant.save()
            messages.success(request, 'Successfully saved :)')
            return render(request, 'gharkakhana/restdetails.html', context)

    elif request.method == 'GET':
        return render(request, 'gharkakhana/details.html')
...
def changePassword(request):
    if request.method == "POST":
        if request.session['type'] == 'customer':
            ...
            return render(request, 'gharkakhana/changePassword.html')
        elif request.session['type'] == 'restaurant':
            restaurant = Restaurant.objects.get(email=request.session['id'])
            oldPassword = request.POST.get('oldPassword')
            newPassword = request.POST.get('newPassword')
...
def restaurantOrderHistory(request):
    ...
    return render(request, 'gharkakhana/resthistory.html', context)
...
def restprofile(request):
    ...
    return render(request, 'gharkakhana/restprofile.html', context)
...
def restview(request, restname):
    if 'id' in request.session.keys():
        try:
            ...
            return render(request, 'gharkakhana/restview.html', context)
...
def cart(request):
    if 'id' in request.session.keys():
        if request.method == 'GET':
            ...
            return render(request, "gharkakhana/ordercart.html", context)
            ...
            for x, y in zip(orders, ordersqty):
                o = Order(customer=customer, restaurant=x, foodlist=orders[x], foodqty=ordersqty[y],
                          ordertime=datetime.datetime.now(), deliverystatus='p')
                o.calamount()
                o.save()
            messages.success(request, "Payment Successfull :)")
            return render(request, "gharkakhana/ordercart.html")
    else:
        return render(request, "gharkakhana/login.html")
...
def history(request):
        if 'id' in request.session.keys():
            ...
            return render(request, "gharkakhana/userhistory.html", context)
        else:
            return render(request, "gharkakhana/login.html")
            ...
            for x in foodall:
                if 'food' + str(x.pk) in request.POST.keys():
                    if int(request.POST['food' + str(x.pk)]) > 0:
                        cartitem = Cart(customer=Customer.objects.get(email=request.session['id']),
                                        fooditem=FoodItem.objects.get(pk=x.pk), foodqty=request.POST['food' + str(x.pk)])
                        cartitem.save()
            customer = Customer.objects.get(email=request.session['id'])
            query = Cart.objects.all()
            cart = {}
...
def addfooditem(request):
        if 'id' in request.session.keys() and request.session['type'] == 'restaurant':
            ...
            food = FoodItem(resid=restaurant, name=name, cuisine=cuisine, price=price, course='s',
                            availability_time=datetime.datetime.now())
            food.save()
